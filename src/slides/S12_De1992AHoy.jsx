...
const TIMELINE = [
  ...
  { year: 1998, label: 'LeCun',        event: 'CNN — LeNet',               color: '#06b6d4',
    detail: 'Redes convolucionales aplicadas a reconocimiento de dígitos postales. La jerarquía de rasgos emerge sola.' },
  { year: 2012, label: 'AlexNet',      event: 'ImageNet (+10%)',            color: '#22c55e',
    detail: 'GPU + datos masivos + dropout. Brecha de 10% con el segundo lugar. Inicio del Deep Learning moderno.' },
  { year: 2017, label: 'Transformers', event: 'Atención',                  color: '#eab308',
    detail: '"Attention is all you need." Mecanismo de atención como recuperación masivamente paralela de representaciones distribuidas.' },
  ...
  { year: 2024, label: 'LLMs',         event: 'Modelos generativos',       color: '#ef4444',
    detail: 'Multimodalidad, razonamiento emergente, agentes. ¿La red ha llegado a ser lo que el cerebro no pudo ser?' },
]
...
export default function S12_De1992AHoy({ profesorMode }) {
  ...
  return (
    ...
    >
      ...
      <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.7rem', width: '100%' }}>
          ...
          {TIMELINE.map((t, i) => {
            ...
            return (
              <motion.div
                ...
                onClick={() => setDetailIdx(isOpen ? null : i)}
                whileHover={{ scale: 1.03, y: -2 }}
                style={{
                  flex: '1 1 140px',
                  backdropFilter: 'blur(12px)',
                  background: isOpen
                    ? `rgba(${hexRgb(t.color)},0.18)`
                    : 'rgba(8,8,22,0.72)',
                  borderTop: `3px solid ${t.color}`,
                  borderRadius: '8px',
                  border: `1px solid ${isOpen ? t.color + 'aa' : t.color + '33'}`,
                  padding: '0.55rem 0.7rem',
                  cursor: 'pointer',
                  transition: 'background 0.2s, border-color 0.2s',
                  boxShadow: isOpen ? `0 0 20px ${t.color}33` : 'none',
                }}
              >
                <div style={{ fontSize: '0.78rem', color: t.color, fontFamily: 'monospace', fontWeight: 700 }}>{t.year}</div>
                <div style={{ fontSize: '0.82rem', fontWeight: 600, color: 'var(--text-h)' }}>{t.label}</div>
                ...
                <AnimatePresence>
                  {isOpen && (
                    ...
                    >
                      <p style={{
                        fontSize: '0.72rem', color: 'var(--text)', lineHeight: 1.5,
        ...
        <div style={{ display: 'flex', gap: '0.5rem', width: '100%', maxWidth: '1100px' }}>
          {REALIZABILIDAD.map(r => (
            ...
            >
              <div style={{ fontSize: '0.72rem', fontWeight: 600, color: r.color }}>{r.titulo}</div>
              <div style={{ fontSize: '0.66rem', color: 'var(--text)', lineHeight: 1.4 }}>{r.t}</div>
            </div>
          ))}
        </div>
        ...
        <motion.div
          ...
          style={{
            ...
            width: '100%',
            textAlign: 'center',
            boxShadow: '0 0 30px rgba(124,109,250,0.1)',
          }}
        >
          <div style={{ fontSize: '0.92rem', color: 'var(--accent-2)', fontStyle: 'italic', lineHeight: 1.55 }}>
        ...
        {profesorMode && (
          <div className="st-card" style={{ maxWidth: '1000px', width: '100%', fontSize: '0.82rem', backdropFilter: 'blur(10px)', background: 'rgba(8,8,22,0.8)' }}>
